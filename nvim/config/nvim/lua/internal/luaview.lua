...
local function luaview(expr)
	...
	for k in expr:gmatch("[^%.]+") do
		v = type(v) == "table" and v[k] or nil
	end
	if type(v) ~= "function" then
		return vim.notify("Not a function", vim.log.levels.ERROR)
	end

	local i = debug.getinfo(v, "S")
	if not i or i.what ~= "Lua" then
		return vim.notify("Not a Lua function", vim.log.levels.ERROR)
	end

	local src = (i.source or ""):gsub("^@", "")
	if src == "" then
		return vim.notify("No source", vim.log.levels.ERROR)
	end
